      ...
      <div className="bg-[#1a1d23] rounded-lg w-full max-w-md">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#3a3d45]">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-white text-lg font-semibold">Confirmar Baja de Usuario</h2>
              <p className="text-gray-400 text-sm">Esta acción se puede revertir</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6">
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button
              ...
              disabled={isLoading}
              className="bg-red-500 hover:bg-red-600 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-6 py-2 rounded-lg font-medium transition-colors flex items-center space-x-2"
            >
              {isLoading && (
                <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
              )}
              <span>{isLoading ? 'Procesando...' : 'Dar de Baja'}</span>
            </button>
          </div>
        </div>
      </div>
