...
import { Loader2, QrCode, CheckCircle, X, Plug } from 'lucide-react';

interface WhatsAppAPIConnectionFormProps {
  onClose: () => void;
}
...
const WhatsAppAPIConnectionForm = ({ onClose }: WhatsAppAPIConnectionFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Éxito",
        description: "Sesión WhatsApp API creada correctamente",
      });
  ...
  const handleQRConnected = async () => {
    ...
    try {
      ...
      if (data?.status === 'connected') {
        toast({
          title: "¡Conectado!",
          description: "WhatsApp API conectado correctamente",
        });
  ...
  const handleDeleteSession = async () => {
    ...
    const confirmed = window.confirm('¿Estás seguro de que quieres eliminar esta sesión de WhatsApp API?');
    if (!confirmed) return;
    ...
    try {
      ...
      toast({
        ...
        description: "La sesión de WhatsApp API ha sido eliminada correctamente",
      });
          ...
          <form onSubmit={handleSubmit} className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ej: WhatsApp API Principal"
                required
              />
            </div>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center space-x-2">
              ...
              <span>Escanear código QR - WhatsApp API</span>
            </DialogTitle>
          </DialogHeader>
...
export default WhatsAppAPIConnectionForm;
